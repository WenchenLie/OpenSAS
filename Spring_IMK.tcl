...
#
# Modified by: Wenchen Lie ↓
# Now one can print the IMK model parameters if "note" is given
#
##################################################################################################################


proc Spring_IMK {SpringID NodeI NodeJ E Fy Ix d htw bftf ry L Ls Lb My PgPye CompositeFlag ConnectionType Units {note ""}} {
...
if {$note ne ""} {
	puts "$note:\nMy: $My_P, theta_p: $theta_p_P, theta_pc: $theta_pc_P, theta_r-theta_c: [expr $theta_pc_P*(1-$Res_P)], theta_u: $theta_u, McMy: $McMyP, Res: $Res_P"
}
