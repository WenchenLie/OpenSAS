...
class DataProcessing:
        ...
        for idx_gm in range(self.GM_N):
            ...
            while True:
                ...
                for story in range(1, self.N + 1):
                    # 遍历楼层
                    # data = np.loadtxt(folder / f'SDR{story}_MF.out')
                    data = pd.read_csv(folder / f'SDR{story}_MF.out', header=None).to_numpy()[:, 0]
                ...
                # roof_d = np.loadtxt(folder/f'Disp{self.N+1}_MF.out')[11:]
                roof_d = pd.read_csv(folder/f'Disp{self.N+1}_MF.out', header=None).to_numpy()[11:, 0]
    ...
    def _read_PFV(self, print_result):
        ...
        for idx_gm in range(self.GM_N):
            ...
            while True:
                ...
                for story in range(1, self.N + 1):
                    # 遍历楼层
                    data = np.loadtxt(folder / f'RFV{story+1}_MF.out')
                    data = data - roof_v_base
                    data_max= max(abs(data))
                    PFV[story - 1] = data_max
                self._mkdir(self.root_out/subfolder)
                np.savetxt(self.root_out/subfolder/'层速度.out', PFV)
                roof_v = np.loadtxt(folder/f'RFV{self.N+1}_MF.out')[11:]
            ...
            while True:
                ...
                for story in range(1, self.N + 1):
                    ...
                    for col in range(1, self.span + 2):
                        ...
                        if col == 1:
                            # 左边柱
                            theta_r = np.loadtxt(folder/f'BeamSpring{story+1}{col}R_D.out')[:, 2]
                            theta_r = max(abs(theta_r))
                            hinge_result[story-1, 0] = theta_r
                        elif col == self.span + 1:
                            # 右边柱
                            theta_l = np.loadtxt(folder/f'BeamSpring{story+1}{col}L_D.out')[:, 2]
            ...
            while True:
                ...
                for story in range(1, self.N + 2):
                    ...
                    for col in range(1, self.span + 2):
                        ...
                        if story == 1:
                            # 首层（地面）
                            theta_T = np.loadtxt(folder/f'ColSpring{story}{col}T_D.out')[:, 2]
                            theta_T = max(abs(theta_T))
                            hinge_result[0, col-1] = theta_T
                        elif story == self.N + 1:
                            # 顶层
                            theta_B = np.loadtxt(folder/f'ColSpring{story}{col}B_D.out')[:, 2]
        ...
        for idx_gm in range(self.GM_N):
            ...
            while True:
                ...
                for story in range(2, self.N + 2):
                    # 遍历楼层 2-5
                    for col in range(1, self.span + 2):
                        theta = np.loadtxt(folder/f'PZ{story}{col}_D.out')
                        theta = max(abs(theta))
    ...
    def read_pushover(self, H: float, truss_length: list=None, FDR_step=[0.01*i for i in range(1, 11)],
                      is_rocking: bool=False, print_result=True, plot_result=True, span_num=1):
        """读取pushover结果，计算应力

        Args:
            H (float): 结构总高度
            truss_length (list): 各层斜撑长度
            FDR_step (list, optional): 不同级别的屋顶位移角，用于计算不同屋顶位移角下结构的归一化层位移
            print_result (bool, optional): 是否打印结果（默认True）
            plot_result (bool, optional): 是否绘制曲线（默认True）
            span_num (int, optional): 摇摆跨的跨数（默认为1）

        Raises:
            ValueError: _description_
        """
        if self.running_case != 'pushover':
            raise ValueError('【Error】文件夹中存放的不是pushover结果')
        ...
        weight /= 2  # 这里不知道为什么要除以2，但这样结果才与FM-2D对得上
        ...
        u = np.zeros((self.N, n))
        for i in range(self.N):
            u[i] = np.loadtxt(self.root/'pushover'/f'Disp{i+2}_MF.out')[11:]
        # 层间位移角
        IDR = np.zeros((self.N, n))
        for i in range(self.N):
            IDR[i] = np.loadtxt(self.root/'pushover'/f'SDR{i+1}_MF.out')[11:]
        # 归一化楼层位移角(各层位移除以结构总高)
        ...
        if is_rocking:
            # 摇摆跨的梁、柱、斜撑内力
            def MyPrint(*str_, end='\n'):
                if print_result:
                    print(*str_, end=end)
            section_lib = {}
            # 1 梁
            MyPrint('各层梁构件端部弯矩应力比(%)：')
            text = ''
            for span in range(1, span_num + 1):
                MyPrint(f'span = {span}:\n', end='')
                text += f'span = {span}:\n'
                for floor in range(2, self.N + 2):
                    data = np.loadtxt(self.root/'pushover'/f'R_Beam{floor}{span}.out')
                    Ni, Nj = data[:, 0], data[:, 3]
                    Mi, Mj = data[:, 2], data[:, 5]
                    section = func.get_rocking_section(self.N, 'Beam', floor=floor, span=span)
                    if section in section_lib:
                        res = section_lib[section]
                    else:
                        res = func.get_rocking_property(section)
                        section_lib[section] = res
                    H, A, Iy, Iz, W = res
                    sgmi_N, sgmj_N = Ni / A, Nj / A
                    sgmi_M, sgmj_M = Mi / W, Mj / W
                    sgmi, sgmj = sgmi_N + sgmi_M, sgmj_N + sgmj_M  # 轴弯组合
                    # sgmi, sgmj = sgmi_M, sgmj_M  # 纯弯
                    sgm = max(max(abs(sgmi)), max(abs(sgmj)))
                    ratio = round(sgm / 0.345 * 100, 2)
                    MyPrint(f'\t{ratio}')
                    text += f'\t{ratio}\n'
            with open(self.root_out/'摇摆跨梁应力比.txt', 'w') as f:
                f.write(text)

            # 2 柱
            MyPrint('各层柱构件端部弯矩应力比(%)：')
            text = ''
            for axis in range(1, span_num*2 + 1):
                MyPrint(f'axis = {axis}:\n', end='')
                text += f'axis = {axis}:\n'
                for story in range(1, self.N + 1):
                    try:
                        data = np.loadtxt(self.root/'pushover'/f'R_Column{story}{axis}.out')
                    except:
                        data = np.array([[0, 0, 0, 0, 0, 0]])
                    Ni, Nj = data[:, 1], data[:, 4]
                    Mi, Mj = data[:, 2], data[:, 5]
                    section = func.get_rocking_section(self.N, 'Column', story=story, axis=axis)
                    if section in section_lib:
                        res = section_lib[section]
                    else:
                        res = func.get_rocking_property(section)
                    ...
                    MyPrint(f'\t{ratio}')
                    text += f'\t{ratio}\n'
            with open(self.root_out/'摇摆跨柱应力比.txt', 'w') as f:
                f.write(text)

            # 3 支撑
            MyPrint('各层斜撑构件应力比(临界失稳应力比)(%)：')
            text = ''
            for span in range(1, span_num + 1):
                MyPrint(f'span = {span}:\n', end='')
                text += f'span = {span}:\n'
                for story in range(1, self.N + 1):
                    try:
                        data_a = np.loadtxt(self.root/'pushover'/f'R_Truss{story}{span}a.out')
                        data_b = np.loadtxt(self.root/'pushover'/f'R_Truss{story}{span}b.out')
                    except:
                        data_a = np.loadtxt(self.root/'pushover'/f'R_Truss{story}{span}a.out')
                        data_b = np.array([0, 0, 0, 0, 0, 0])
                    Na, Nb = max(abs(data_a)), max(abs(data_b))
                    Na_c, Nb_c = min(data_a), min(data_b)
                    section = func.get_rocking_section(self.N, 'Truss', story=story, span=span)
                    if section in section_lib:
                        res = section_lib[section]
                    else:
                        res = func.get_rocking_property(section)
                    ...
                    H, A, Iy, Iz, W = res
                    sgma, sgmb = Na / A, Nb / A
                    sgma_c, sgmb_c = Na_c / A, Nb_c / A
                    miu = 1  # 长度系数
                    i = pow(Iy / A, 0.5)
                    lamda = miu * truss_length[story-1] / i
                    sgm_cr = pi**2 * 206 / lamda**2
                    ra_strength, rb_strength = round(sgma / 0.345 * 100, 2), round(sgmb / 0.345 * 100, 2)
                    ra_stable, rb_stable = round(-sgma_c / sgm_cr * 100, 2), round(-sgmb_c / sgm_cr * 100, 2)
                    MyPrint(f'\ta:{ra_strength}({ra_stable})  b:{rb_strength}({rb_stable})')
                    text += f'\ta:{ra_strength}({ra_stable})  b:{rb_strength}({rb_stable})\n'
                else:
                    MyPrint('')
            with open(self.root_out/'摇摆跨支撑应力比.txt', 'w') as f:
                f.write(text)
        logger.success('完成')
    ...
    def read_th(self):
        ...
        IDR.to_csv(self.root_out/'结果统计'/'层间位移角.csv', encoding='ANSI')
        CIDR.to_csv(self.root_out/'结果统计'/'累积层间位移角.csv', encoding='ANSI')
        PFV.to_csv(self.root_out/'结果统计'/'层速度(mm_s).csv', encoding='ANSI')
        PFA.to_csv(self.root_out/'结果统计'/'层加速度(g).csv', encoding='ANSI')
        RIDR.to_csv(self.root_out/'结果统计'/'残余层间位移角.csv', encoding='ANSI')
        IDR_stat.to_csv(self.root_out/'结果统计'/'层间位移角_统计.csv', encoding='ANSI')
        CIDR_stat.to_csv(self.root_out/'结果统计'/'累积层间位移角_统计.csv', encoding='ANSI')
        PFV_stat.to_csv(self.root_out/'结果统计'/'层速度_统计.csv', encoding='ANSI')
        PFA_stat.to_csv(self.root_out/'结果统计'/'层加速度_统计.csv', encoding='ANSI')
        RIDR_stat.to_csv(self.root_out/'结果统计'/'残余层间位移角_统计.csv', encoding='ANSI')
        beam_hinge_stat_mean.to_csv(self.root_out/'结果统计'/'梁铰_统计_mean.csv', encoding='ANSI')
        beam_hinge_stat_std.to_csv(self.root_out/'结果统计'/'梁铰_统计_std.csv', encoding='ANSI')
        beam_hinge_stat_16th.to_csv(self.root_out/'结果统计'/'梁铰_统计_16th.csv', encoding='ANSI')
        beam_hinge_stat_50th.to_csv(self.root_out/'结果统计'/'梁铰_统计_50th.csv', encoding='ANSI')
        beam_hinge_stat_84th.to_csv(self.root_out/'结果统计'/'梁铰_统计_84th.csv', encoding='ANSI')
        col_hinge_stat_mean.to_csv(self.root_out/'结果统计'/'柱铰_统计_mean.csv', encoding='ANSI')
        col_hinge_stat_std.to_csv(self.root_out/'结果统计'/'柱铰_统计_std.csv', encoding='ANSI')
        col_hinge_stat_16th.to_csv(self.root_out/'结果统计'/'柱铰_统计_16th.csv', encoding='ANSI')
        col_hinge_stat_50th.to_csv(self.root_out/'结果统计'/'柱铰_统计_50th.csv', encoding='ANSI')
        col_hinge_stat_84th.to_csv(self.root_out/'结果统计'/'柱铰_统计_84th.csv', encoding='ANSI')
        panel_zone_stat_mean.to_csv(self.root_out/'结果统计'/'节点域_统计_mean.csv', encoding='ANSI')
        panel_zone_stat_std.to_csv(self.root_out/'结果统计'/'节点域_统计_std.csv', encoding='ANSI')
        panel_zone_stat_16th.to_csv(self.root_out/'结果统计'/'节点域_统计_16th.csv', encoding='ANSI')
        panel_zone_stat_50th.to_csv(self.root_out/'结果统计'/'节点域_统计_50th.csv', encoding='ANSI')
        panel_zone_stat_84th.to_csv(self.root_out/'结果统计'/'节点域_统计_84th.csv', encoding='ANSI')
