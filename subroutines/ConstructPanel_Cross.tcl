...
proc ConstructPanel_Cross {Axis Floor X_Axis Y_Floor E A_Panel I_Panel  d_Col d_Beam transfTag ShapeID} {
	...
	node $NodeCL 		  $X_Axis            		$Y_Floor;  # central node
	node $Node_XY01 [expr $X_Axis]            [expr $Y_Floor - $d_Beam/2];  # bottom node
	if {$ShapeID != 2 && $ShapeID != 23} {
	node $Node_XY02 [expr $X_Axis - $d_Col/2] [expr $Y_Floor];  # left node
	}
	if {$ShapeID != 3 && $ShapeID != 23 && $ShapeID != 34} {
	node $Node_XY03 [expr $X_Axis]            [expr $Y_Floor + $d_Beam/2];  # top node
	}
	if {$ShapeID != 4 && $ShapeID != 34} {
	node $Node_XY04 [expr $X_Axis + $d_Col/2] [expr $Y_Floor];  # right node
	}
