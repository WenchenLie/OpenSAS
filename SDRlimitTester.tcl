...
proc SDRlimitTester {numStories SDRlimit MFFloorNodes EGFFloorNodes h1 htyp TraceGFDrift GMname} {
...
# set RunTime [expr $x - $StartTime];
	...
	for {set i 0} {$i <= $numStories-1} {incr i} {
		...
		if { $SMFTDrift > $SDRlimit || $GFTDrift > $SDRlimit} {
			...
			set filename1 [format "%s_CollapseState.txt" $GMname];  # TODO
			set fileID [open $filename1 w];
